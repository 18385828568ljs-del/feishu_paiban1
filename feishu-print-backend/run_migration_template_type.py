"""
数据库迁移脚本：为 templates 表添加 template_type 字段
"""
import sys
import io
...
def run_migration():
    """执行数据库迁移"""
    print("=" * 60)
    print("开始执行数据库迁移：添加 template_type 字段")
    print("=" * 60)
    ...
    try:
        with engine.connect() as connection:
            ...
            try:
                ...
                if column_info:
                    print("[提示] template_type 字段已存在，跳过添加字段步骤")
                    ...
                    # 执行迁移：添加字段
                    print("正在添加 template_type 字段...")
                    alter_sql = text("""
                        ALTER TABLE templates 
                        ADD COLUMN template_type VARCHAR(20) DEFAULT 'normal' NOT NULL 
                        AFTER content
                    """)
                ...
                # 为现有数据设置默认值
                print("正在更新现有数据的 template_type 值...")
                update_sql = text("""
                    UPDATE templates 
                    SET template_type = 'normal' 
                    WHERE template_type IS NULL OR template_type = ''
                """)
                result = connection.execute(update_sql)
                updated_rows = result.rowcount
                print(f"[成功] 已更新 {updated_rows} 条记录的 template_type 值")
                print()
                ...
                if not index_exists:
                    # 添加索引
                    print("正在添加索引 idx_template_type...")
                    create_index_sql = text("""
                        CREATE INDEX idx_template_type ON templates(template_type)
                    """)
                    connection.execute(create_index_sql)
                    print("[成功] 索引添加成功！")
                    print()
                else:
                    print("[提示] 索引 idx_template_type 已存在，跳过创建索引步骤")
                    print()
